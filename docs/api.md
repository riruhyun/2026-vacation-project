...
기본 경험치에 조건별 보너스를 얹는 구조입니다. 사용자가 카드 획득 화면에서 받은 이유를 바로 읽을 수 있고, 나중에 계절 관찰이나 미션을 붙일 때 규칙을 갈아엎지 않아도 됩니다.
...
- **희귀도 보너스는 첫 발견에만 붙습니다.** 재관찰에도 주면 드문 식물 한 종을 찾아낸 뒤 그것만 반복 촬영하는 쪽이 새 종을 찾는 것보다 이득이 됩니다.
- **새 종 하나 ≈ 재관찰 열 번**입니다. 이 서비스가 유도하려는 행동이 새로운 종을 찾는 것이므로 격차를 크게 벌려 두었습니다.

같은 날 판정은 **한국 시간 기준**입니다. `observed_at`은 UTC로 저장되므로 그대로 자르면 오전 9시 이전이 전날로 취급됩니다.

### 레벨

`레벨 N → N+1 필요 XP = 100 + 50 × (N - 1)`

`100 → 150 → 200 → 250 …`

첫 식물을 찍으면 바로 Lv.2가 됩니다. 첫 카드에서 보상을 크게 체감시키고 이후 서서히 느려집니다. 카드 50장(흔함 16 / 보통 13 / 드묾 21)을 다 모으면 6,375 XP로 **Lv.15**에 닿습니다.

**저장은 누적 XP 하나만 합니다.** 레벨은 누적 XP에서 다시 계산합니다. `profiles.level` 컬럼도 있지만 DB 함수가 예전 곡선으로 채우므로 **서버는 이 값을 읽지 않습니다.**

응답의 `reward.breakdown`이 지급 근거를 항목별로 내려줍니다.
...
보상이 없으면 `breakdown`은 빈 배열입니다. 지급액과 항목 합이 어긋나는 경우에도 비웁니다 — 틀린 근거를 보여주지 않기 위해서입니다.

저장 시 Supabase 함수 `record_collection_observation_reward`가 발견 횟수, XP, 관찰 기록을 한 트랜잭션에서 함께 갱신합니다. 이 함수는 팀 전체가 공유하므로 건드리지 않았습니다. 대신 함수가 값을 2^재관찰횟수로 나누는 것을 감안해, 서버가 미리 곱해서 넘깁니다(`lib/progress.ts`의 `toBaseXp`).
